...
if test_conn:
    with st.spinner("Testing connections..."):
        ...
        for ex in exchanges:
            status = "OK" if ex in test_clients else "FAIL"
...
if run or auto:
    ...
    if total_calls > 500:
        st.warning(
            f"{len(exchanges)} exchanges x {len(symbols)} symbols = {total_calls} API calls. "
    ...
    with st.spinner(f"Scanning {len(symbols)} symbols x {len(clients)} exchanges..."):
        t0 = time.time()
    ...
    if not diagnostic_mode and (not opps or all(o["net bps"] <= 0 for o in opps)):
        ...
        st.markdown("""
        **This is normal.** Efficient crypto markets have extremely tight spreads. Here is why and what to do:
